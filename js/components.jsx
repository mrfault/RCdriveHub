...
function Badge({ children, tone = 'neutral', icon, dot = false, solid = false, cut = false, style, ...rest }) {
...
function Button({ children, variant = 'primary', size = 'md', icon, trailingIcon, cut = false, block = false, disabled = false, className = '', style, ...rest }) {
...
function IconButton({ name, variant = 'ghost', size = 'md', round = false, count, label, style, ...rest }) {
...
/* ---- Logo ---- */
function Logo({ size = 28, mark = true, mono, style, ...rest }) {
...
/* ---- SearchBar ---- */
function SearchBar({ placeholder = 'Model və ya hissə nömrəsi axtar...', size = 'md', defaultValue = '', onSubmit, style, ...rest }) {
...
/* ---- FilterChip ---- */
function FilterChip({ children, active = false, count, removable = false, icon, onClick, onRemove, style, ...rest }) {
  return (
...
/* ---- QuantityStepper ---- */
function QuantityStepper({ value = 1, min = 1, max = 99, onChange, size = 'md', style, ...rest }) {
...
/* ---- SpecRow ---- */
function SpecRow({ label, value, icon, accent = false, style, ...rest }) {
  return (
...
/* ---- ProductCard ---- */
function ProductCard({ image, title, brand, price, oldPrice, badge, rating, scale, onAdd, style, ...rest }) {
...
Object.assign(window, { Icon, Badge, Button, IconButton, Logo, SearchBar, FilterChip, Tabs, QuantityStepper, SpecRow, ProductCard, Lightbox });
